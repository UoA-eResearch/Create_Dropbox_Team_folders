...
class Dropbox
  ...
  # @silent [Boolean] If false, then print result of the query to stdout
  # @return [Array<Hash>] Dropbox response is a JSON array, which we parse, and return as a Ruby Array.
  def dropbox_query(query:, query_data: '{}', silent: true)
    WebBrowser::https_session(host: DROPBOX_API_SERVER, verify_cert: false) do |wb|
      ...
      begin
        ...
        h = JSON.parse(r)
        puts JSON.pretty_generate(h) if !silent
        return h
        ...
        if e.web_return_code == 429 #Too Many Requests
          retry_count += 1
          sleep retry_count
          return dropbox_query(query: query, query_data: query_data, silent: silent, retry_count: retry_count) if retry_count <= 4
        end
  ...
  def team_list(silent: true)
    r = dropbox_query(query: '2/team/members/list', silent: silent)
    r["members"].each do |g|
      yield g
    end
    while r["has_more"] 
      r = dropbox_query(query: '2/team/members/list/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent)
      r["members"].each do |g|
        yield g
      end
    end
  ...
  def groups_list(silent: true)
    r = dropbox_query(query: '2/team/groups/list', silent: silent)
    r["groups"].each do |g|
      yield g
    end
    while r["has_more"] 
      r = dropbox_query(query: '2/team/groups/list/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent)
      r["groups"].each do |g|
        yield g
      end
    end
  ...
  # @return group_id
  def group_id(group_name:, silent: true)
    groups_list(silent: silent) do |gf|
  ...
  def team_folder_list(silent: true)
    r = dropbox_query(query: '2/team/team_folder/list',silent: silent)
    r["team_folders"].each do |n|
      yield n
    end
    while r["has_more"] 
      r = dropbox_query(query: '2/team/team_folder/list/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent)
      r["team_folders"].each do |n|
        yield n
      end
    end
  ...
  def team_folder_create(folder:, silent: true)
    dropbox_query(query: '2/team/team_folder/create', query_data: "{\"name\":\"#{folder}\"}", silent: silent)
  end
  
  #Find a team folders ID from its name (not something Dropbox API does)
  # @param folder_name [String] 
  # @return team_folder_id
  def team_folder_id(folder_name:, silent: true)
    team_folder_list(silent: silent) do |tf|
  ...
  def namespaces_list(silent: true)
    r = dropbox_query(query: '2/team/namespaces/list',silent: silent)
  ...
  def get_events(silent: true)
    r = dropbox_query(query: '2/team_log/get_events',silent: silent)
    r["events"].each do |n|
      yield n
    end
    while r["has_more"] 
      r = dropbox_query(query: '2/team_log/get_events/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent)
      r["events"].each do |n|
        yield n
      end
    end
  ...
  # @silent [Boolean] If false, then print result of the query to stdout
  def team_add_members(members_details:, send_welcome: true, silent: true ) #email:, given_name:, surname:, external_id: )
    ...
    (0..members_details.length).step(20) do |i|
      ...
      dropbox_query(query: '2/team/members/add', query_data: member_query, silent: silent)
    end
  end
  ...
  # @silent [Boolean] If false, then print result of the query to stdout
  def group_create(group_name:, silent: true)
    group_query = "{\"group_name\":\"#{group_name}\",\"group_external_id\":\"#{group_name}\",\"group_management_type\":{\".tag\":\"company_managed\"}}"
    dropbox_query(query: '2/team/groups/create', query_data: group_query, silent: silent)
  end
  ...
  def group_add_members(external_group_id: nil, group_id: nil, emails:, silent: true)
    members = []
    emails.each do |e|
    ...
    #puts group_query
    dropbox_query(query: '2/team/groups/members/add', query_data: group_query, silent: silent)
  end
  
  def group_remove_members(external_group_id: nil, group_id: nil, emails:, silent: true)
    members = []
    emails.each do |e|
    ...
    dropbox_query(query: '2/team/groups/members/remove', query_data: group_query, silent: silent)
  end
  ...
  def group_members_list(external_group_id: nil, group_id: nil, silent: true)
    if group_id != nil
    ...
    r = dropbox_query(query: '2/team/groups/members/list', query_data: group_query, silent: silent)
    r["members"].each do |m|
      yield m
    end
    while r["has_more"] 
      r = dropbox_query(query: '2/team/groups/members/list/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent)
      r["members"].each do |m|
        yield m
      end
    end
  ...
  # @silent [Boolean] If false, then print result of the query to stdout
  def members_get_info(members: , silent: true)
    members_json = []
    members.each do |m|
    ...
    dropbox_query(query: '2/team/members/get_info', query_data: "{\"members\":[#{members_json.join(',')}]}", silent: silent)
  end
  ...
  def group_get_info(external_group_ids: nil, group_ids: nil, silent: true)
    if group_ids != nil
    ...
    dropbox_query(query: '2/team/groups/get_info', query_data: group_query, silent: silent)
  end
  
  def list_folder_members(folder_id: , silent: true)
    r = dropbox_query(query: '2/sharing/list_folder_members', query_data: "{\"shared_folder_id\":\"#{folder_id}\"}", silent: silent) 
    h = { "users" => r["users"], "groups" => r["groups"], "invitees" => r["invitees"] }
    yield h
    #Needs loop if too many users or groups 
    while r["cursor"]
      r = dropbox_query(query: '2/sharing/list_folder_members/continue', query_data: "{\"cursor\":\"#{r["cursor"]}\"}", silent: silent) 
      h = { "users" => r["users"], "groups" => r["groups"], "invitees" => r["invitees"] }
      yield h
    end
  end
  ...
  def add_group_folder_member(folder_id:, group_id:, access_role: "editor", custom_message: nil, silent: true)
    ...
    dropbox_query(query: '2/sharing/add_folder_member', query_data: query_data, silent: silent) 
  end
  ...
  # @param access_role [String] "editor" by default. "viewer" for read only access
  def add_user_folder_member(folder_id:, user_id: nil, email: nil, access_role: "editor", custom_message: nil, silent: true)
    if email != nil
  ...
  def team_members_set_profile(team_member_id: nil, email: nil, external_id: nil, given_name: nil, surname: nil, new_external_id: nil, new_email: nil, silent: true)
    ...
    if id != nil && query_data_a.length > 1
      ...
      dropbox_query(query: '2/team/members/set_profile', query_data: query_data_a.join(',') + '}', silent: silent) 
    end
  end
end
